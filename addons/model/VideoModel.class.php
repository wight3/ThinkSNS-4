...
class VideoModel extends Model {
	...
	public function upload($from=0,$timeline=0){
		...
		if( $uploadCondition ){
			...
			if( !file_exists( $sourceSavePath ) ) mkdir( $sourceSavePath, 0777, true );
			if( !file_exists( $partSavePath ) ) mkdir( $partSavePath, 0777, true );
			$filename = uniqid();    //文件名称
			$image_name = $filename.'.jpg';  
			$video_source_name = $filename.'.'.$video_ext;   //源视频名称
			$video_name = $filename.'.mp4';   //视频名称
			if(@move_uploaded_file($_FILES['video']['tmp_name'], $sourceSavePath.'/'.$video_source_name))   //上传视频到源视频文件夹
			{
				set_time_limit(0);
	...
	public function upload_by_web($from=0){
		...
		if( $uploadCondition ){
			...
			$video_source_name = $filename.'.'.$video_ext;  //源视频名称
			$video_name = $filename.'.mp4';  //视频名称
			if(@move_uploaded_file($_FILES['Filedata']['tmp_name'], $sourceSavePath.'/'.$video_source_name))   //上传到source文件夹
			{
				set_time_limit(0);
				if(PATH_SEPARATOR==':'){  //Linux
					$ffmpegpath = $video_config['ffmpeg_path'];
	...
	function mobile_video_codec($ffmpegpath,$sourceSavePath,$video_source_name,$savePath,$video_name){
	...
	function get_video_timeline($ffmpegpath, $input){
	...
	function get_video_image($ffmpegpath, $input, $output, $fromdurasec="01"){
		if(!file_exists($input)) return false;
		if(!$ffmpegpath){
			$data = model('Xdata')->get('admin_Content:video_config');
			if(PATH_SEPARATOR==':'){  //Linux
	...
	function get_video_part($ffmpegpath, $input, $output, $begin_second="01", $end_second="05" ){
		if(!file_exists($input)) return false;
		if(!$ffmpegpath){
			$data = model('Xdata')->get('admin_Content:video_config');
			if(PATH_SEPARATOR==':'){  //Linux
